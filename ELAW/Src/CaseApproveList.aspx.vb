...
Partial Class Src_CaseApproveList
    ...
    Dim DS As DataSet
    Dim iRec As Integer
    Const menuID As Integer = Constant.MenuID.CaseApproveList
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Session("CaseAppList") Is Nothing Then
                Me.gData()
            Else
                DVLst = Session("CaseAppList")
            End If
    ...
    Private Sub gData(Optional ByVal Type As String = "")
        ...
        strsql &= " case when c.status_id in (select step2 from case_process) then c.app1 else c.app2 end app,"
        strsql &= " a.menu_id, a.status_id, a.assign_from, a.assign_to"
        ...
        strsql &= " on c.creation_by=e.empid left join   "
        strsql &= " AUTHORIZE a "
        ' strsql &= " (select * from AUTHORIZE where assign_from ='" & sEmpNo & "') a"
        strsql &= " on c.status_id =a.status_id and a.menu_id =30 and a.status_id in (select step2 from case_process union select step3 from case_process) "
        strsql &= " where (c.active = 1 "
        strsql &= " and c.status_id in (select step2 from case_process union select step3 from case_process) "
        strsql &= " and case when c.status_id in (select step2 from case_process) then c.app1 else c.app2 end='" & sEmpNo & "' "
        strsql &= " or (a.assign_from = case when c.status_id in (select step2 from case_process) then c.app1 else c.app2 end and a.assign_to ='" & sEmpNo & "' and c.active =1 "
    ...
    Protected Sub GridView1_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridView1.RowEditing
        ...
        Response.Redirect("../Src/CasePreview.aspx?id=" & K1(0) & "&status=approve&menu=3")
    End Sub
