...
Partial Class Src_SetAlertDate
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            If Session("set_alert") Is Nothing Then
                Me.gData()
            Else
                DV = Session("set_alert")
            End If
        End If
        txtStatus.Attributes.Add("onkeypress", "return blockNonNumbers(this, event,-1,0);")
    ...
    Private Sub Auto()
        ...
        sqlTmp = "SELECT TOP 1 prefix_id FROM prefix "
        sqlTmp &= " ORDER BY prefix_id DESC"
        ...
        Try
            With comTmp
                ...
                tmpMemberID2 = drTmp.Item("prefix_id")
    ...
    Private Sub gData()
        ...
        strsql = "select c.chk_id,c.chk_name,c.chk_date,f.name creation_by,c.created_date,  "
        strsql &= "f1.name updated_by,c.updated_date "
        strsql &= "from check_alert c inner join fullname f "
        strsql &= "on c.creation_by=f.empid inner join fullname f1 "
        strsql &= "on c.updated_by=f1.empid "

        If txtStatus.Text <> "" Then
            strsql &= " where chk_name like '%" & txtStatus.Text & "%'"
        End If

        Dim DT As DataTable = MD.GetDataTable(strsql)
        DV = DT.DefaultView
        DV.Sort = "[" & ViewState("sortfield") & "] " & ViewState("sortdirection")
        Session("set_alert") = DV
    End Sub
    Private Sub MyGridBind()
        GridView1.DataSource = DV
        Dim X1() As String = {"chk_id"}
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridView1.RowDeleting
        ...
        Dim Y As Integer = MD.Execute("delete from prefix where prefix_id ='" & K1(0) & "'")
        If Y = 1 Then
    ...
    Protected Sub GridView1_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridView1.RowEditing
        ...
        Dim lDate As Label = GridView1.Rows(e.NewEditIndex).Cells(1).Controls(1)

        lblId.Text = K1(0).ToString
        lblType.Text = lName.Text
        txtStatus.Text = lDate.Text
    ...
    Protected Sub bSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bSave.Click
        ...
        If lblStatus.Text = "Edit" Then
            If txtStatus.Text.Trim = "" Then
                MC.MessageBox(Me, "กรุณากรอกจำนวนวันที่แจ้งเตือน")
                txtStatus.Focus()
                Exit Sub
            End If
            'Edit Data
            Try
                strsql = "update check_alert set "
                strsql &= "chk_date=?,updated_by=?,updated_date=? "
                strsql &= "where chk_id=?"
        ...
        Else
            MC.MessageBox(Me, "กรุณาเลือกข้อมูลที่ต้องการแก้ไข")

            'Me.Auto()
            ''Insert data

            'Try
            '    strsql = "insert into prefix (prefix_id,prefix_name,creation_by,created_date,updated_by,updated_date)"
            '    strsql &= " Values (?,?,?,?,?,?)"
