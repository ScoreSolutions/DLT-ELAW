...
Imports System.IO
Partial Class Src_ContractPreviewList
    ...
    Dim DS As DataSet
    Dim iRec As Integer
    Const menuID As Integer = Constant.MenuID.ContractPreviewList
    Private Sub SetSearchSession()
        ...
        dt.Columns.Add("txtDateFrom1")
        dt.Columns.Add("txtDateTo1")

        dt.Columns.Add("txtNo")
        dt.Columns.Add("txtMatetial")
        dt.Columns.Add("txtContractName")
        ...
        data("txtDateFrom1") = txtDateFrom1.Text
        data("txtDateTo1") = txtDateTo1.Text

        data("txtNo") = txtNo.Text
        data("txtMatetial") = txtMatetial.Text
        data("txtcontractName") = txtContractName.Text
    ...
    Private Sub FillCondition()
        If Session(Constant.searchSession) IsNot Nothing Then
            ...
            If data("menuID") = menuID Then
                ...
                txtDateFrom1.Text = data("txtDateFrom1")
                txtDateTo1.Text = data("txtDateTo1")

                txtNo.Text = data("txtNo")
                txtMatetial.Text = data("txtMatetial")
                txtContractName.Text = data("txtContractName")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Session("ContractPreviewLst") Is Nothing Then
                Me.gData()
            Else
                DVLst = Session("ContractPreviewLst")
            End If
    ...
    Public Sub DataStatus()
        'สถานะสัญญา
        Dim strsql As String
        strsql = "select status_id,status_name from contract_status order by status_name  "
    ...
    Private Sub gData(Optional ByVal Type As String = "")
        'ดึงข้อมูลมาแสดงใน Gridveiw
        Dim strsql As String

        strsql = "select d.contract_id,d.contract_no,d.dates_recieve,d.dates_contract,d.dates_comesign,d.dates_sign, "
        strsql &= "d.material,d.user_sale,d.tax_id,d.subtype_id,d.user_buy, "
        strsql &= "case when d.contract_name is not null then s.subtype_name+'('+convert(nvarchar(200),d.contract_name)+')' else s.subtype_name end subtype_name,"
        ...
        If ddlStatus.SelectedValue <> "0" Then
            If ddlStatus.SelectedValue = "8" Then
                strsql &= "and d.cancel_id is not null "
            Else
        ...
        If txtKeyword.Text <> "" Then
            ...
            strsql &= "or e1.firstname +' '+e1.lastname like '%" & txtKeyword.Text & "%')  "
        End If

        If txtDateFrom.Text.Year <> 1 And txtDateTo.Text.Year <> 1 Then
            strsql &= " and convert(nvarchar(10),d.dates_recieve,120) between convert(nvarchar(10),'" & txtDateFrom.SaveDate & "',120) and convert(nvarchar(10),'" & txtDateTo.SaveDate & "',120)  "
        End If

        If txtDateFrom1.Text.Year <> 1 And txtDateTo1.Text.Year <> 1 Then
            strsql &= " and convert(nvarchar(10),d.dates_sign,120) between convert(nvarchar(10),'" & txtDateFrom1.SaveDate & "',120) and convert(nvarchar(10),'" & txtDateTo1.SaveDate & "',120)  "
        End If

        If txtContractName.Text <> "" Then
            strsql &= "and d.contract_name like '%" & txtContractName.Text & "%'"
        End If

        Dim DT As DataTable = MD.GetDataTable(strsql)
        DVLst = DT.DefaultView
        DVLst.Sort = "[" & ViewState("sortfield") & "] " & ViewState("sortdirection")
        Session("ContractPreviewLst") = DVLst
    ...
    Protected Sub GridView1_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridView1.RowEditing
        ...
        Response.Redirect("../Src/ContractPreview.aspx?id=" & K1(0) & "&status=preview&menu=4")
    End Sub
