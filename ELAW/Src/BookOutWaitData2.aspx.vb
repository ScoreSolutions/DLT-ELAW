...
Partial Class Src_BookOutWaitData2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If X <> "" Then
                ...
                sql &= "b.sendto1,b.sendto_app,b.sendto_comment,b.sendto1_app,b.sendto_comment1,t.type_name,b.ref_title,b.ref_id,b.ref_type,b.creation_by "
    ...
    Public Sub MyDataBind()
        ...
        'If DS.Tables(0).Rows(0).Item("sendto2_app").ToString = "T" Then
        rdoApp.SelectedValue = DS.Tables(0).Rows(0).Item("sendto1_app").ToString
        'End If
        ...
        If rdoApp.SelectedValue = "F" Or rdoApp.SelectedValue = "H" Then
            Me.DataSendTo()
    ...
    Public Sub DataSendTo()
        ...
        If rdoApp.SelectedValue <> "H" Then
            strsql = "select f.empid,f.name namesend from fullname f inner join employee e   "
            strsql &= "on f.empid=e.empid where e.dept_id=1 "
        Else
            strsql = "select f.empid,p.pos_name namesend from fullname f inner join employee e   "
            strsql &= "on f.empid=e.empid inner join position p on e.pos_id=p.pos_id "
            strsql &= "where e.dept_id=1 and e.pos_id in (2,3,5,6)"
        End If

        Dim DTS As DataTable = MD.GetDataTable(strsql)
        Dim dr As DataRow = DTS.NewRow
        dr!empid = 0
        dr!namesend = "---------โปรดเลือก---------"
        DTS.Rows.InsertAt(dr, 0)
        ddlSendName.DataTextField = "namesend"
    ...
    Protected Sub bSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bSave.Click
        ...
        If rdoApp.SelectedValue <> "T" And rdoApp.SelectedValue <> "F" And rdoApp.SelectedValue <> "H" Then
        ...
        Try
            Dim app As String = Me.CheckApprove("sendto1", "8")

            strsql = "update bookout_data set sendto1_app=?,sendto_comment1=?, "
            strsql &= "updated_by=?,updated_date=?,sendto_ass2=?,sendto1_date=? "
            strsql &= "where bookout_id=? "


            cn = New OleDbConnection(MD.Strcon)
            cmd = New OleDbCommand(strsql, cn)
            MD.CreateParam(cmd, "TTTDTDT")
            ...
            Else
                cmd.Parameters("@P5").Value = System.DBNull.Value
            End If
            cmd.Parameters("@P6").Value = Date.Now
    ...
    Protected Sub bSaveAndSend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bSaveAndSend.Click
        ...
        If rdoApp.SelectedValue <> "T" And rdoApp.SelectedValue <> "F" And rdoApp.SelectedValue <> "H" Then
            ...
            Exit Sub
        End If
        If rdoApp.SelectedValue = "F" And rdoApp.SelectedValue = "H" Then
            If ddlSendName.SelectedValue = "0" Then
                lblChkApp.Text = ""
                lblChkComment.Text = ""
                lblChkSend.Text = "กรุณาเลือกชื่อผู้ที่จะส่งต่อ"
                Exit Sub
            End If
        End If
        If txtComment.Text = "" Then
        ...
        If rdoApp.SelectedValue = "T" Then
            chkState = "9"
            Sname = DS.Tables(0).Rows(0).Item("creation_by").ToString
            Hname = DS.Tables(0).Rows(0).Item("sendto").ToString
        ElseIf rdoApp.SelectedValue = "H" Then
            chkState = "7"
            Sname = DS.Tables(0).Rows(0).Item("creation_by").ToString
            Hname = ddlSendName.SelectedValue
        Else
            chkState = "6" 'ส่งให้เป็นเจ้าของเรื่องสถานะเริ่มต้น 
            Sname = ddlSendName.SelectedValue
            Hname = DS.Tables(0).Rows(0).Item("sendto").ToString
        End If
        ...
        If app = "1" Then
            ...
            strsql &= "sendto1,sendto1_app,sendto_comment1,sendto_ass1,sendto_ass2,"
            strsql &= "creation_by,created_date,updated_by,updated_date,ref_type,ref_id,ref_title,booktype_id,sendto_date,sendto1_date,runno) "
            ...
            strsql &= "message,postscript,postname,post_pos,contact,comment,'" & Hname & "',sendto_app,sendto_comment, "
            strsql &= "sendto1,'" & rdoApp.SelectedValue & "','" & txtComment.Text & "',sendto_ass1,'" & sEmpNo & "', "
            strsql &= "'" & Sname & "',created_date,'" & sEmpNo & "',getdate(),ref_type,ref_id,ref_title,booktype_id,sendto_date,getdate(),runno  "
            strsql &= "from bookout_data where bookout_id='" & lblId.Text & "'"
        Else
            strsql = "insert into bookout_data (bookout_id,system_no,dates,bookkind_id, "
            strsql &= "user_request,user_sign,status_id,priority_id,keyword,topic,present, "
            strsql &= "message,postscript,postname,post_pos,contact,comment,sendto,sendto_app,sendto_comment, "
            strsql &= "sendto1,sendto1_app,sendto_comment1,"
            strsql &= "creation_by,created_date,updated_by,updated_date,ref_type,ref_id,ref_title,booktype_id,sendto_date,sendto1_date,runno) "
            strsql &= "select '" & lblIdNew.Text & "',system_no,dates,bookkind_id,   "
            strsql &= "user_request,user_sign,'" & chkState & "',priority_id,keyword,topic,present, "
            strsql &= "message,postscript,postname,post_pos,contact,comment,'" & Hname & "',sendto_app,sendto_comment, "
            strsql &= "sendto1,'" & rdoApp.SelectedValue & "','" & txtComment.Text & "', "
            strsql &= "'" & Sname & "',created_date,'" & sEmpNo & "',getdate(),ref_type,ref_id,ref_title,booktype_id,sendto_date,getdate(),runno  "
            strsql &= "from bookout_data where bookout_id='" & lblId.Text & "'"
        End If
        MD.Execute(strsql)
