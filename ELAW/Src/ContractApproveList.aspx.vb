...
Partial Class Src_ContractApproveList
    ...
    Dim DS As DataSet
    Dim iRec As Integer
    Const menuID As Integer = Constant.MenuID.ContractApproveList
    Private Sub ChkPermis()
        Dim sEmpNo As String = HttpContext.Current.User.Identity.Name
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Session("ContractAppList") Is Nothing Then
                Me.gData()
            Else
                DVLst = Session("ContractAppList")
            End If
    ...
    Private Sub gData(Optional ByVal Type As String = "")
        ...
        'sql += " where menu_id=35 "
        'sql += " and status_id = 2 "
        'sql += " and assign_to='" & sEmpNo & "' "
        'sql += " and convert(nvarchar(10),getdate(),120) between convert(nvarchar(10),date_from,120) and convert(nvarchar(10),date_to,120)"

        'Dim vAssignFrom As String = ""
        'Dim dt1 As DataTable = MD.GetDataTable(sql)
        'If dt1.Rows.Count > 0 Then
        '    vAssignFrom = dt1.Rows(0)("assign_from").ToString()
        'End If

        'Dim sql2 As String = ""
        'sql2 += " select * from authorize "
        'sql2 += " where menu_id=35 "
        'sql2 += " and status_id =3 "
        'sql2 += " and assign_to='" & sEmpNo & "' "
        'sql2 += " and convert(nvarchar(10),getdate(),120) between convert(nvarchar(10),date_from,120) and convert(nvarchar(10),date_to,120) "

        'Dim vAssignFrom2 As String = ""
        'Dim dt2 As DataTable = MD.GetDataTable(sql2)
        'If dt2.Rows.Count > 0 Then
        '    vAssignFrom2 = dt2.Rows(0)("assign_from").ToString()
        'End If


        'strsql = "select d.contract_id,d.contract_no,d.dates_recieve,d.dates_contract,d.dates_comesign,d.dates_sign, "
        'strsql &= "d.material,d.user_sale,d.tax_id,d.subtype_id,d.witness1 witness,d.status_id,d.cancel_id,d.creation_by,d.keyword,d.active,"
        'strsql &= "case when convert(nvarchar(10),d.dates_recieve,120) <= convert(nvarchar(10),getdate()-4,120)and d.cancel_id is null "
        'strsql &= "then 'SHOW' else 'NO' end warning, "
        'strsql &= "e1.firstname +' '+e1.lastname creation_name,s.subtype_name,s1.status_name "
        ...
        'strsql &= "where ((d.status_id=2 and d.cancel_id is null and d.witness1='" & sEmpNo & "' or d.witness1='" & vAssignFrom & "') and d.active=1 and d.witness1_app is null  "
        'strsql &= "or (d.status_id=3 and d.cancel_id is null and d.witness2='" & sEmpNo & "' or d.witness2='" & vAssignFrom2 & "') and d.active=1)  "
        ...
        strsql &= " then 'SHOW' else 'NO' end warning,"
        strsql &= " e1.firstname +' '+e1.lastname creation_name,s.subtype_name,s1.status_name,"
        strsql &= " case when d.status_id =2 then d.witness1 else d.witness2 end app"
        strsql &= " from contract_data d"
        strsql &= " inner join contract_subtype s"
        strsql &= " on d.subtype_id=s.subtype_id"
        strsql &= " inner join employee e1"
        ...
        strsql &= " left join  "
        strsql &= " AUTHORIZE a "
        'strsql &= " (select * from AUTHORIZE where assign_from ='" & sEmpNo & "') a"
        strsql &= " on d.status_id =a.status_id and a.menu_id =35 and a.status_id in (2,3) "
        strsql &= " where(d.active = 1 "
        strsql &= " and d.status_id in (2,3) "
        strsql &= " and case when d.status_id =2 then d.witness1 else d.witness2 end='" & sEmpNo & "' "
        strsql &= " or (a.assign_from = case when d.status_id =2 then d.witness1 else d.witness2 end and a.assign_to ='" & sEmpNo & "' and d.active =1 "
    ...
    Protected Sub GridView1_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles GridView1.RowEditing
        ...
        Response.Redirect("../Src/ContractPreview.aspx?id=" & K1(0) & "&status=app&menu=4")
    End Sub
