...
Public Class Counter
    ...
    Public Shared Function setLawCounter(ByVal LawID As String) As Boolean
        ...
        Return (MD.Execute(sql) > 0)
    End Function
    Public Shared Function DocCounter(ByVal TypeID As String, ByVal SubTypeID As String) As Boolean
        'เริ่มนับเมื่อมีผู้ดาวน์โหลดกฎหมาย
        Dim MD As New MainData
        Dim dt As DataTable = MD.GetDataTable("select top 1 qty from doc_counter where doc_type='" & TypeID & "' and doc_subtype='" & SubTypeID & "'")
        Dim sql As String = ""
        If dt.Rows.Count > 0 Then
            sql += "update doc_counter "
            sql += " set qty = " & Convert.ToDouble(dt.Rows(0)("qty")) + 1
            sql += " ,updated_date=getdate() "
            sql += " where doc_type='" & TypeID & "' and doc_subtype='" & SubTypeID & "' "
        Else
            'เพิ่มข้อมูลให้กรณีที่ประชาชนดาวน์โหลดกฎหมายนี้เป็นคนแรก (ถือเป็นเกียรติอย่างสูงเลยนะเนี่ยะ ^o^)
            sql += "insert into doc_counter "
            sql += " values('" & TypeID & "','" & SubTypeID & "',1,getdate() )"
        End If
