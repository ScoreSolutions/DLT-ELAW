...
Partial Class SearchLaw
    ...
    Private Sub gData(Optional ByVal Type As String = "", Optional ByVal TypeContract As String = "", Optional ByVal TypeCase As String = "")
        ...
        If txtKeyword.Text.Trim <> "" Then
            ...
            ' ''strsql &= " where d.active = 1 and d.secret=0 "

            ' ''If Type <> "" Then
            ' ''    strsql &= " and t.doc_id=1 and v.type_id  in (" & Type & ") and  d.active = 1 and d.secret=0  "
            ...
            ' ''        strsql &= " and "
            ' ''    End If
            ' ''    strsql &= " t.doc_id=3 and v.type_id  in (" & TypeContract & ")  And d.active = 1 and d.secret=0 "
            ' ''    strsql &= setKeyWord(txtKeyword.Text)
            ' ''End If
            ...
            ' ''        End If
            ' ''    End If
            ' ''    strsql &= "t.doc_id=2 and v.type_id  in (" & TypeCase & ") And d.active = 1 and d.secret=0 "
            ' ''    strsql &= setKeyWord(txtKeyword.Text)
            ' ''End If

            ' ''strsql &= "and (k.keyword like '%" & txtKeyword.Text & "%' or d.doc_name like '%" & txtKeyword.Text & "%') "
            ...
            ' ''strsql &= " select d.doc_id,case when d.cancel=1 then d.doc_name+' (ยกเลิก : '+d.cancel_comment+')' else d.doc_name end doc_name,"
            ' ''strsql &= " v.name subtype_name,d.doc_type ,d.doc_subtype,'0' inx "
            ' ''strsql &= " from import_document d  "
            ' ''strsql &= " inner join import_keywords k"
            ' ''strsql &= " on d.doc_id=k.doc_id inner join DOCUMENT_TYPE t"
            ' ''strsql &= " on d.doc_type=t.doc_id  "
            ...
            'แสดงรายการเดียวที่ค้นหาเจอก่อน
